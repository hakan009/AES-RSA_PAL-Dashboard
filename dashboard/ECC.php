<?php
class ECC {
    private $publicKey;
    private $privateKey;

    // Constructor takes ECC keys instead of RSA keys
    public function __construct($publicKey, $privateKey) {
        $this->publicKey = $publicKey;
        $this->privateKey = $privateKey;
    }

    // ECC encryption method using public key
    public function encrypt($data) {
        openssl_public_encrypt($data, $encryptedData, $this->publicKey, OPENSSL_ALGO_SHA256); // Use ECC with SHA256 for signing
        return base64_encode($encryptedData);
    }

    // ECC decryption method using private key
    public function decrypt($encryptedData) {
        openssl_private_decrypt(base64_decode($encryptedData), $decryptedData, $this->privateKey, OPENSSL_ALGO_SHA256);
        return $decryptedData;
    }
}

// Load ECC keys
$privateKey = "-----BEGIN EC PRIVATE KEY-----\n 
\n-----END EC PRIVATE KEY-----";
$publicKey = "-----BEGIN PUBLIC KEY-----\
UwIDAQAB\n-----END PUBLIC KEY-----";

$ecc = new ECC($publicKey, $privateKey);

$encrypted = $ecc->encrypt("Hello, world!");
echo "Encrypted: $encrypted\n";

$decrypted = $ecc->decrypt($encrypted);
echo "Decrypted: $decrypted\n";


?>
