...
session_start();
include "../config.php";
include "ECC.php"; // Make sure ECC class is correctly included

if (isset($_POST['encrypt_now'])) {
    $user          = $_SESSION['username'];
    $deskripsi     = mysqli_escape_string($connect, $_POST['desc']);

    // Generate ECC keys or use existing keys
...
-----END PUBLIC KEY-----";
    ...
    if (is_uploaded_file($file_tmpname)) {
        ...
        while ($data = fread($file_source, 117)) { // Adjust to ECC max encryption block size
            $cipher = $ecc->encrypt($data);
